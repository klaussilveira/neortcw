...
#define CLIENT_WINDOW_TITLE "Wolfenstein: Enemy Territory"
#endif
...
// Stubs for legacy X11 GLX code references in renderer
...
static int GLimp_SetMode(int mode, qboolean fullscreen, qboolean noborder)
{
	...
	for (i = 0; i < 16; i++)
	{
		...
		if( fullscreen )
		{
			...
			mode.refresh_rate = glConfig.displayFrequency = ri.Cvar_Get( "r_displayRefresh", "0", 0 )->integer;
...
static qboolean GLimp_StartDriverAndSetMode(int mode, qboolean fullscreen, qboolean noborder)
{
	...
	if (fullscreen && ri.Cvar_Get( "in_nograb", "0", 0 )->integer )
	{
...
static void GLimp_InitExtensions( void )
{
	...
	if ( SDL_GL_ExtensionSupported( "GL_EXT_texture_filter_anisotropic" ) )
	{
		...
		{
			GLint maxAniso = 0;
			qglGetIntegerv( GL_MAX_TEXTURE_MAX_ANISOTROPY_EXT, &maxAniso );
			if ( maxAniso <= 0 )
			{
	...
	{
		if ( ri.Cvar_Get( "r_ext_ATI_pntriangles", "0", 0 )->integer )
		{
			qglPNTrianglesiATI = SDL_GL_GetProcAddress( "glPNTrianglesiATI" );
			qglPNTrianglesfATI = SDL_GL_GetProcAddress( "glPNTrianglesfATI" );
			...
			else
			{
				qglPNTrianglesiATI = NULL;
				qglPNTrianglesfATI = NULL;
				ri.Printf( PRINT_ALL, "...GL_ATI_pn_triangles failed to load\n" );
...
void GLimp_Init( void )
{
	...
	if( ri.Cvar_Get( "com_abnormalExit", "0", 0 )->integer )
	{
...
success:
...

	{
		...
		if ( ext )
			Q_strncpyz( glConfig.extensions_string, ext, sizeof( glConfig.extensions_string ) );
		else
...
void GLimp_EndFrame( void )
{
	...
	if( r_fullscreen->modified )
	{
		...
		if( r_fullscreen->integer && ri.Cvar_Get( "in_nograb", "0", 0 )->integer )
		{
			ri.Printf( PRINT_ALL, "Fullscreen not allowed with in_nograb 1\n");
			ri.Cvar_Set( "r_fullscreen", "0" );
			r_fullscreen->modified = qfalse;
